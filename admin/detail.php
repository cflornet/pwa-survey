						...
						<div class="card-body">
							...
							<div class="table-responsive">
								<table id="tbp" class="table">
									...
									<tbody>
										<? 	
                                            $sql1 = "SELECT DATE_ADD(didf_hour,INTERVAL usen_utc HOUR) hour,didc_description FROM sdia_diary,sdid_diary_detail,buse_user where diac_student = didc_student AND diaf_date = didf_date AND diac_student = usec_id AND usec_id = ".$_GET['cod'].";";
											$trns = $pdo -> query($sql1);
											while ($trn = $trns -> fetch()) 
											{
  												echo "<tr>
  															<td>".$trn['hour']."</td>
															  <td>".$trn['didc_description']."</td>
  													  </tr>";
  											}
										?>
									</tbody>
